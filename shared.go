...
func getViafBatch(service string, input []string) (map[string]string, error) {
...
// results on `results`.
func viafWorker(id int, jobs <-chan []string, results chan<- batchResult) {
	for j := range jobs {
		var br batchResult
		switch j[0] {
		case "lcn":
